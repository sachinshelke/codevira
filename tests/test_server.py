...
for _mod_name in _modules_to_mock:
    ...
    # is unnecessary — and a fake `mcp.types` with a partial symbol set leaks
    # into the whole pytest session, breaking later `from mcp.types import
    # Resource` in test_decision_replay / test_qa_round_week13. Fake ONLY as a
    # fallback for environments where the real package can't be imported.
    try:
...
# TextContent / Resource leak into every later test) and breaks tests that
# exercise the real MCP protocol, e.g. tests/test_binding_e2e.py. Mirrors
# the tree_sitter "only stub when not already loaded" guard below.
...
class TestCallToolDispatch:
    def test_dispatch_get_roadmap(self):
        ...
        with (
            patch("mcp_server.server.get_roadmap", return_value=sentinel) as mock_fn,
    ...
    def test_dispatch_get_node(self):
        ...
        with (
            patch("mcp_server.server.get_node", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_add_phase(self):
        ...
        with (
            patch("mcp_server.server.add_phase", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            result = _run(
                call_tool(
    ...
    def test_dispatch_get_impact(self):
        ...
        with (
            patch("mcp_server.server.get_impact", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
...
class TestCallToolExceptionHandling:
    ...
    def test_exception_in_tool_with_crash_logger(self):
        ...
        with (
            patch("mcp_server.server.get_roadmap", side_effect=ValueError("bad")),
            patch("mcp_server.auto_init.ensure_project_initialized"),
...
class TestCallToolAutoInit:
    def test_ensure_project_initialized_called(self):
        ...
        with (
            patch("mcp_server.server.get_roadmap", return_value={"ok": True}),
    ...
    def test_auto_init_failure_does_not_block_dispatch(self):
        ...
        sentinel = {"phase": 1}
        with (
            patch("mcp_server.server.get_roadmap", return_value=sentinel),
            patch(
...
class TestCrashLoggerResilience:
    def test_crash_logger_failure_does_not_break_dispatch(self):
        ...
        with (
            patch("mcp_server.server.get_node", side_effect=RuntimeError("boom")),
            patch("mcp_server.auto_init.ensure_project_initialized"),
            patch(
                "mcp_server.crash_logger.log_crash",
                side_effect=Exception("logger broken"),
            ),
        ):
...
class TestCallToolSerialization:
    def test_dict_result_serialized_as_json(self):
        ...
        with (
            patch("mcp_server.server.get_roadmap", return_value=data),
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            result = _run(call_tool("get_roadmap", {}))
        parsed = json.loads(result[0].text)
    ...
    def test_result_is_text_content_type(self):
        ...
        with (
            patch("mcp_server.server.get_roadmap", return_value={}),
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            result = _run(call_tool("get_roadmap", {}))
        assert result[0].type == "text"
    ...
    def test_result_is_list_of_one_element(self):
        ...
        with (
            patch("mcp_server.server.get_roadmap", return_value={"a": 1}),
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            result = _run(call_tool("get_roadmap", {}))
        assert len(result) == 1
...
class TestCallToolAdditionalRoutes:
    ...
    def test_dispatch_complete_phase(self):
        ...
        with (
            patch("mcp_server.server.complete_phase", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            result = _run(
                call_tool(
    ...
    def test_dispatch_get_playbook(self):
        ...
        with (
            patch("mcp_server.server.get_playbook", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_refresh_graph(self):
        ...
        with (
            patch("mcp_server.server.refresh_graph", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_get_history(self):
        ...
        with (
            patch("mcp_server.server.get_history", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_defer_phase(self):
        ...
        with (
            patch("mcp_server.server.defer_phase", return_value=sentinel) as mock_fn,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            result = _run(
                call_tool(
...
class TestCallToolMissingDispatches:
    ...
    def test_dispatch_update_phase_status(self):
        sentinel = {"status": "updated"}
        with (
            patch("mcp_server.server.update_phase_status", return_value=sentinel) as m,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_defer_phase(self):
        ...
        with (
            patch("mcp_server.server.defer_phase", return_value=sentinel) as m,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_complete_phase(self):
        ...
        with (
            patch("mcp_server.server.complete_phase", return_value=sentinel) as m,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
            _run(
                call_tool(
    ...
    def test_dispatch_get_phase(self):
        ...
        with (
            patch("mcp_server.server.get_phase", return_value=sentinel) as m,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_refresh_graph(self):
        ...
        with (
            patch("mcp_server.server.refresh_graph", return_value=sentinel) as m,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
    ...
    def test_dispatch_query_graph(self):
        sentinel = {"callees": []}
        with (
            patch("mcp_server.server.query_graph_tool", return_value=sentinel) as m,
            patch("mcp_server.auto_init.ensure_project_initialized"),
        ):
...
class TestServerMain:
    def test_main_runs_background_startup_tasks(self):
        ...
        with (
            ...
            patch(
                "mcp_server.paths.get_project_root", return_value=MagicMock()
            ) as mock_root,
        ):
    ...
    def test_main_installs_crash_handler(self):
        with (
            patch("mcp_server.crash_logger.install_global_handler") as mock_handler,
            patch("asyncio.run"),
            patch(
                "indexer.index_codebase.start_background_watcher",
                return_value=MagicMock(),
