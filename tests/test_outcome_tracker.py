...
class TestAnalyzeSessionOutcomes:
    ...
    @patch("indexer.outcome_classifier.classify_outcome")
    def test_specific_session_id(
        self, mock_classify, mock_root, mock_data_dir, populated_db
    ):
        ...
        # P17: surfaces delegate to the shared classifier; mock it.
        mock_classify.return_value = "kept"
    ...
    def test_none_session_processes_all_without_outcomes(
        ...
        mock_classify.return_value = "kept"  # P17: mock the shared classifier
    ...
    def test_session_with_no_decisions(
...
class TestFileOutcomeClassification:
    ...
    def test_file_kept_no_subsequent_commits(
        ...
        """Classifier says 'kept' -> outcome 'kept' is recorded."""
        project, data_dir, db = _get_db(populated_db)
        mock_data_dir.return_value = data_dir
        mock_root.return_value = project
...
class TestGitErrorHandling:
    ...
    def test_git_cmd_returns_none_on_called_process_error(self):
        ...
        with patch(
            "indexer.outcome_tracker._project_root", return_value=Path("/tmp/fake")
        ):
            with patch(
    ...
    def test_git_failure_during_analysis_is_graceful(
        ...
        """A git failure must not crash, and must NOT record a bogus outcome.

        P17: the shared classifier returns None on git failure (can't classify)
        — strictly more correct than the old optimistic "kept". The contract
        is now: no crash, and no outcome recorded.
        """
        project, data_dir, db = _get_db(populated_db)
        mock_data_dir.return_value = data_dir
        mock_root.return_value = project
        ...
        # Patches the GLOBAL subprocess.check_output, so the shared classifier's
        # git calls raise too → classify_outcome returns None.
        mock_git.side_effect = subprocess.CalledProcessError(128, "git")

        from indexer.outcome_tracker import analyze_session_outcomes

        analyze_session_outcomes(session_id="s_err")  # must not raise

        outcomes = db.get_outcomes_for_file("src/err_file.py")
        assert len(outcomes) == 0  # unclassifiable → no outcome, not a fake "kept"
