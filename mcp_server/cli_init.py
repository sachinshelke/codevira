...
def cmd_init(*, yes: bool = False, dry_run: bool = False, shared: bool = False) -> int:
    ...
    if gitignore_blocks_codevira:
        ...
        print("     other AI tools (Cursor, Antigravity, etc.) won't see any of")
    ...
    if not gitignore_has_cache:
        ...
        addition = (
            ...
            ".codevira-cache/\n"
            # Derived state, ignored for the SAME reason as the cache: both are
            # pure functions of .codevira/decisions.jsonl and are regenerated
            # by `codevira sync` / any record_decision. In a team repo
            # (`init --shared`) committing them means every concurrent write
            # is a merge conflict in generated content nobody reviews —
            # measured: two branches each recording one decision produced
            # conflicts in both files while the canonical decision log merged
            # cleanly via the codevira merge driver.
            #
            # AGENTS.md is deliberately NOT here. It is derived too, but it is
            # the contract Codex/Copilot read WITHOUT running codevira, so
            # ignoring it would trade a cross-tool promise for a merge
            # conflict. It stays committed.
            "\n# Codevira derived state (regenerated by `codevira sync`;\n"
            "# ignored so concurrent writes don't collide on generated files)\n"
            ".codevira/digest.jsonl\n"
            ".codevira/manifest.yaml\n"
        )
