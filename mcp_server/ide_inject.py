...
def _portable_hook_command(cmd_path: str) -> str:
    """Build the enforcement-hook command, preferring a PATH-relative form.

    ``.agents/hooks.json`` lives INSIDE the repo, so Antigravity treats it as
    workspace config a team may commit. Embedding an absolute path
    (``/Users/alice/.local/bin/codevira``) means every teammate — and the same
    user on another OS — inherits a hook pointing at one machine's home dir,
    silently breaking enforcement (D00011Y).

    So: if ``codevira`` resolves on PATH, emit the BARE command — portable
    across macOS / Linux / Windows and safe to commit. ``shutil.which`` also
    finds ``codevira.exe`` on Windows. Only when it is NOT on PATH do we fall
    back to the absolute path (or the ``python -m mcp_server`` fallback), which
    is machine-specific but at least works locally; that file is gitignored for
    this repo and regenerated by ``codevira init``.
    """
    suffix = "engine handle --ide antigravity PreToolUse"
    if shutil.which("codevira") is not None:
        return f"codevira {suffix}"
    # Not on PATH. If cmd_path is the Python-interpreter fallback, invoke the
    # module; otherwise the codevira binary directly. Quote for spaces in paths.
    # Split on BOTH separators: a Windows cmd_path may be generated on Windows
    # but this code (and its tests) also runs on posix, where Path.name would
    # not strip a backslash-delimited prefix.
    base = re.split(r"[\\/]", cmd_path)[-1].lower()
    if base.startswith("py"):
        return f'"{cmd_path}" -m mcp_server {suffix}'
    return f'"{cmd_path}" {suffix}'
            ...
            "PreToolUse": [
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": _portable_hook_command(cmd_path),
                            "timeout": 15,
                        }
                    ],
                }
