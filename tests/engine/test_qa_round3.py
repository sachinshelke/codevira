...
class TestMCPCallToolWiresEngine:
    ...
    def test_call_tool_source_imports_pre_call(self):
        ...
        assert (
            "from mcp_server.engine.wiring.mcp_dispatch import pre_call" in server_src
        )
    ...
    def test_call_tool_source_imports_post_call(self):
        ...
        assert (
            "from mcp_server.engine.wiring.mcp_dispatch import post_call" in server_src
        )
    ...
    def test_engine_failure_does_not_break_call_tool(self):
        ...
        pre_idx = next(
            (i for i, ln in enumerate(lines) if "pre_call(name, arguments)" in ln),
            -1,
        )
        ...
        for j in range(pre_idx - 1, max(0, pre_idx - 50), -1):
            ...
            if indent < pre_indent:  # different control structure — stop
                break
        assert found_try, "pre_call(name, arguments) must be inside a try/except"
...
class TestHookFastPath:
    ...
    HOOK_DIR = Path(__file__).parent.parent.parent / "mcp_server" / "data" / "hooks"
    ...
    def test_fast_path_runs_in_under_100ms(self):
        ...
        # Warmup
        subprocess.run(
            [bash, str(hook)], input="", capture_output=True, env=env, timeout=5
        )
        # Measure 10 runs
        elapsed_samples = []
        for _ in range(10):
...
class TestSizeCapDocsMatch:
    ...
    def test_spec_documents_actual_value(self):
        spec = (
            Path(__file__).parent.parent.parent / "docs" / "heroes" / "00-engine.md"
        ).read_text()
